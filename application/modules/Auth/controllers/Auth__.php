...
class Auth extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->success_msg = '<div class="alert alert-success alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'.$this->lang->line('label_data_user_create_success').'</div>';
		$this->success_edit = '<div class="alert alert-success alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'.$this->lang->line('label_data_user_edit_success').'</div>';
        $this->fail_msg = '<div class="alert alert-warning alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">'.$this->lang->line('label_data_submit_fail').'</div>';
    ...
    public function users($id=''){
		...
		if($id > 0){
			...
			$sdata = $this->session->userdata('sdata');
			...
			$sdata['group_id'] = $this->input->post('group_id');
				
		}

		$this->session->set_userdata('sdata',$sdata);
		
        $total_rows = $this->option->get_user_list($sdata, NULL, NULL);
        $paginate_data = $this->paginate($total_rows, NULL);
        $records = $this->option->get_user_list(
            $sdata, $paginate_data['per_page'], $paginate_data['page']
        );

        $this->data = array(
            'head' => 'teacher',
            'links' => $paginate_data['link'],
            'records' => $records,
            'total_row' => $total_rows
        );
		
		$this->data['groups'] = $this->Option_model->get_group_options();
    ...
    public function create_user()
    {
        ...
        $this->data['divisions'] = $this->Option_model->get_division_options();
        $this->data['districts'] = $this->Option_model->get_district_option($this->input->post('division_id'));
        $this->data['upazilas'] = $this->Option_model->get_upazila_option($this->input->post('district_id'));
        $this->data['unions'] = $this->Option_model->get_union_option($this->input->post('upazila_id'));
        $this->data['institutes'] = $this->Option_model->get_institute_option_by_union($this->input->post('upazila_id'),$this->input->post('union_id'));
		$this->data['city_corporation'] = $this->Option_model->get_corporation_options();
		$this->data['thanas'] = $this->Option_model->get_thana_options();
		$this->data['paurasabhas'] = $this->Option_model->get_paurasabha_options();
		...
		if($this->input->post('group_id') == 13){
			$this->form_validation->set_rules('division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
			$this->form_validation->set_rules('paurasabha_id', $this->lang->line('label_paurasabha'), 'required');
		}
		if($this->input->post('group_id') == 9){
			$this->form_validation->set_rules('division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
			$this->form_validation->set_rules('corporation_id', $this->lang->line('label_city_corporation'), 'required');
		}
		...
		if($this->input->post('group_id') == 14){
			$this->form_validation->set_rules('division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
			
		}
		if($this->input->post('group_id') == 15){
			$this->form_validation->set_rules('division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
			
		}
		if($this->input->post('group_id') == 16){
			$this->form_validation->set_rules('division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
		...
		if($this->input->post('group_id') == 12){
			...
			$this->form_validation->set_rules('district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
			$this->form_validation->set_rules('union_id', $this->lang->line('label_union'), 'required');
		}
		if($this->input->post('group_id') == 6){
			...
			$this->form_validation->set_rules('upazila_id', $this->lang->line('label_upazila'), 'required');
			$this->form_validation->set_rules('union_id', $this->lang->line('label_union'), 'required');
			$this->form_validation->set_rules('institute_id', $this->lang->line('label_institute'), 'required');
		}
		...
		if($this->input->post('group_id') == 13){
			$additional_data['division_id'] = $this->input->post('division_id');
            $additional_data['district_id'] = $this->input->post('district_id');
            $additional_data['upazila_id'] = $this->input->post('upazila_id');
			$additional_data['paurasabha_id'] = $this->input->post('paurasabha_id');			
			}
		if($this->input->post('group_id') == 9){
			$additional_data['division_id'] = $this->input->post('division_id');
            $additional_data['district_id'] = $this->input->post('district_id');
            $additional_data['corporation_id'] = $this->input->post('corporation_id');
			$additional_data['upazila_id'] = $this->input->post('upazila_id');
		}
		if($this->input->post('group_id') == 12){
			$additional_data['division_id'] = $this->input->post('division_id');
            $additional_data['district_id'] = $this->input->post('district_id');
            $additional_data['upazila_id'] = $this->input->post('upazila_id');
			$additional_data['union_id'] = $this->input->post('union_id');
		}
		if($this->input->post('group_id') == 6){
			$additional_data['division_id'] = $this->input->post('division_id');
            $additional_data['district_id'] = $this->input->post('district_id');
            $additional_data['upazila_id'] = $this->input->post('upazila_id');
			$additional_data['union_id'] = $this->input->post('union_id');
			$additional_data['institute_id'] = $this->input->post('institute_id');
		}
		if($this->input->post('group_id') == 8){
			$additional_data['division_id'] = $this->input->post('division_id');
        ...
        if ($this->form_validation->run() == true && ($user_id = $this->ion_auth->register($identity, $password, $email, $additional_data))) {
			...
			$this->session->set_flashdata(
                    'message', sprintf($this->success_msg, 'School', 'updated')
                );
    ...
    public function edit_user($id = '')
    {
        ...
        $this->data['group_options'] = $this->Option_model->get_group_options();
        $this->data['divisions'] = $this->Option_model->get_division_options();
        $this->data['districts'] = $this->Option_model->get_district_option($school_details->division_id);
        $this->data['upazilas'] = $this->Option_model->get_upazila_option($school_details->district_id);
        $this->data['unions'] = $this->Option_model->get_union_option($school_details->upazila_id);
        $this->data['institutes'] = $this->Option_model->get_institute_option_by_union($school_details->upazila_id,$school_details->union_id);
		$this->data['city_corporation'] = $this->Option_model->get_corporation_options();
		$this->data['thanas'] = $this->Option_model->get_thana_options();
		$this->data['paurasabhas'] = $this->Option_model->get_paurasabha_options();
		...
		if($this->input->post('group_id') == 13){
			$this->form_validation->set_rules('pau_division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('pau_district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('pau_paurasabha_id', $this->lang->line('label_paurasabha'), 'required');
		}
		if($this->input->post('group_id') == 9){
			$this->form_validation->set_rules('cor_division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('cor_district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('cor_corporation_id', $this->lang->line('label_city_corporation'), 'required');
		}
		if($this->input->post('group_id') == 11){
			$this->form_validation->set_rules('up_division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('up_district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('up_upazila_id', $this->lang->line('label_upazila'), 'required');
			
		}
		if($this->input->post('group_id') == 12){
			$this->form_validation->set_rules('un_division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('un_district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('un_upazila_id', $this->lang->line('label_upazila'), 'required');
			$this->form_validation->set_rules('un_union_id', $this->lang->line('label_union'), 'required');
		}
		if($this->input->post('group_id') == 8){
			$this->form_validation->set_rules('d_division_id', $this->lang->line('label_division'), 'required');
		}
		...
		if($this->input->post('group_id') == 6){
			$this->form_validation->set_rules('tech_division_id', $this->lang->line('label_division'), 'required');
			$this->form_validation->set_rules('tech_district_id', $this->lang->line('label_district'), 'required');
			$this->form_validation->set_rules('tech_upazila_id', $this->lang->line('label_upazila'), 'required');
			$this->form_validation->set_rules('tech_union_id', $this->lang->line('label_union'), 'required');
			$this->form_validation->set_rules('institute_id', $this->lang->line('label_institute'), 'required');
		}
		if($this->input->post('group_id') == 14){
			$this->form_validation->set_rules('up_division_id', $this->lang->line('label_division'), 'required');
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($this->form_validation->run() === TRUE) {
					...
					if($this->input->post('group_id') == 13){
						$data['division_id'] = $this->input->post('pau_division_id');
						$data['district_id'] = $this->input->post('pau_district_id');
						$data['upazila_id'] = $this->input->post('pau_upazila_id');
						$data['paurasabha_id'] = $this->input->post('pau_paurasabha_id');			
						}
					if($this->input->post('group_id') == 9){
						$data['division_id'] = $this->input->post('cor_division_id');
						$data['district_id'] = $this->input->post('cor_district_id');
						$data['corporation_id'] = $this->input->post('cor_corporation_id');
						
					}
					if($this->input->post('group_id') == 12){
						$data['division_id'] = $this->input->post('un_division_id');
						$data['district_id'] = $this->input->post('un_district_id');
						$data['upazila_id'] = $this->input->post('un_upazila_id');
						$data['union_id'] = $this->input->post('un_union_id');
					}
					if($this->input->post('group_id') == 8){
						$data['division_id'] = $this->input->post('d_division_id');
					
					}
					if($this->input->post('group_id') == 11){
						$data['division_id'] = $this->input->post('up_division_id');
						$data['district_id'] = $this->input->post('up_district_id');
						$data['upazila_id'] = $this->input->post('up_upazila_id');
					...
					if($this->input->post('group_id') == 6){
						$data['division_id'] = $this->input->post('tech_division_id');
						$data['district_id'] = $this->input->post('tech_district_id');
						$data['upazila_id'] = $this->input->post('tech_upazila_id');
						$data['union_id'] = $this->input->post('tech_union_id');
						$data['institute_id'] = $this->input->post('institute_id');
					}
					if($this->input->post('group_id') == 14){
						$data['division_id'] = $this->input->post('up_division_id');
						$data['district_id'] = $this->input->post('up_district_id');
					...
					$data['create_by'] = $this->ion_auth->user()->row()->id;
                ...
                if ($this->ion_auth->update($user->id, $data)) {
                    ...
                    //$this->session->set_flashdata('message', $this->ion_auth->messages());
					$this->session->set_flashdata(
                    'message', sprintf($this->success_edit, 'School', 'updated')
                );
    ...
    {
        $this->data['groups'] = $this->db->query("select * from groups where id not in (1,2,3,4)")->result_array();
    ...
    public function edit_group($id)
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($group_update) {
                    ...
                    $this->permission->update_menu_permission($id, $user_id,$menu_list, $assign_menu_list);
        ...
        $this->data['user_options'] = $this->permission->get_user_options($id);
