...
class Permission_model extends CI_model
{
    ...
    public function update_menu_permission($group_id, $user_id, $menu_list, $assign_menu_list)
    {
        foreach ($menu_list as $key => $menu) {
            if (in_array($menu, $assign_menu_list)) {
				
                $sql = "UPDATE menu_list
                      set user_id = if(find_in_set($user_id,user_id),
                      user_id,
                      CONCAT(user_id, ',', $user_id)
                      ), group_id = if(find_in_set($group_id,group_id),
                      group_id,
                ...
                $sql = "UPDATE menu_list
                            SET user_id=replace(user_id, ',$user_id','') group_id=replace(group_id, ',$group_id','')
                            WHERE id=$menu
                            ";
            }
            $this->db->query($sql);
    ...
    public function get_user_options($id){
		...
		$this->db->select('id,CONCAT(first_name, " ", last_name)  full_name');
        $query = $this->db->where('group_id',$id)->get('users');
        $data[''] = 'Select Group';
        foreach($query->result_array() as $row) {
            $data[$row['id']] = ucfirst($row['full_name']);
        }
