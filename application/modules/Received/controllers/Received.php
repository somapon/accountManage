...
class Received extends MX_Controller
{
    function __construct()
    {
        ...
        $this->load->model('Received_model', 'received');
    ...
    { 
        $records = $this->received->getVendorList();

        $this->data = array(
            'head'              => 'Received List (RL)',
            'records'           => $records,
		...
		if (isset($_POST) && !empty($_POST) && $this->form_validation->run($this) == TRUE) {
            ...
            if ( ! empty( $_FILES['vendor_img']['name'] ) ){
                ...
                if ( ! $this->upload->do_upload('vendor_img') ) {
                   ...
                   if( $this->received->saveVendorInfo($data) ){
			...
			$this->template->write_view('content', 'new_received', $form_data, TRUE);
            $this->template->render();
		}
	}
	...
	/* public function vendor_edit($id){
		...
		$vinfo = $this->received->getVendorById($id);
		...
		if (isset($_POST) && !empty($_POST) && $this->form_validation->run($this) == TRUE) {
			...
			if ($_FILES['vendor_img']['name']) {
				...
				$this->received->updateVendorInfo($data);
	...
	public function vendor_view($id){
		...
		$vInfo = $this->received->getVendorDetails($id);
