...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, group_id,
		                    last_login, CONCAT(first_name," ",last_name) as full_name')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	*/
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name,'.$this->tables['groups'].'.acl, '.$this->tables['groups'].'.description, user_id')
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db->insert($this->tables['users_groups'], array( $this->join['groups'] => (float)$this->input->post('group_id'), $this->join['users'] => (float)$user_id)))
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			...
			$this->set_error('Update Unsuccessful');
		...
		$this->set_message('Update Uuccessful');
		return TRUE;
	}
	...
	**/
	...
	{
		$this->db->select('u.username, u.image, u.first_name, u.last_name, u.group_id, u.email, u.phone, g.name group_name');
		...
		public function userList($where0,$where1,$where2,$where3,$start){
