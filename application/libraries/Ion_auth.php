...
class Ion_auth
{
    ...
    /*public function user_exist_any_table($id = '')
    {
        $sql = "SELECT TABLE_NAME as tables
                FROM information_schema.COLUMNS 
                WHERE    TABLE_SCHEMA = 'meshop_db' 
        ...
        if (count($result) > 0) {
            foreach ($result as $val) {
                $sql2 = "SELECT user_id FROM " . $val->tables . " WHERE user_id=" . $id;
                $user_id = $this->db->query($sql2)->num_rows();
                if ($user_id > 0)
                    return true;
            }
        } else {
    ...
    public function delete_user_accounts($id='')
    {
        $sql = "DELETE FROM accoutns WHERE user_id=" . $id;
        $this->db->query($sql);
        return true;
    }

    public function delete_user_voucher_settings($id='')
    {
        $sql = "DELETE FROM voucher_settings WHERE user_id=" . $id;
        $this->db->query($sql);
        return true;
    }
